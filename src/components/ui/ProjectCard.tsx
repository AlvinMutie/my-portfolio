...
export function ProjectCard({ project, index }: { project: GitHubRepo; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative p-10 bg-surface border-2 border-white/5 group-hover:border-primary/40 transition-all duration-500 flex flex-col min-h-[420px]">
        <div className="flex justify-between items-start mb-12">
          <div className="flex flex-wrap gap-3">
            {tags.map((tag, j) => (
              <span 
                key={j} 
                className="px-4 py-1.5 text-[10px] font-black uppercase tracking-widest border border-white/10 group-hover:border-primary/40 transition-colors"
              >
                {tag}
              </span>
            ))}
          </div>
          <div className="flex gap-4 text-zinc-500 group-hover:text-primary transition-colors">
            {project.stargazers_count > 0 && (
              <div className="flex items-center gap-1 mr-2 bg-white/5 px-2 py-1 rounded">
        ...
        <div className="space-y-4 mb-auto">
          ...
          <p className="text-lg text-zinc-500 leading-relaxed font-medium line-clamp-3 italic">
            {project.description || "Experimental system architecture and code craft."}
          </p>
        </div>
